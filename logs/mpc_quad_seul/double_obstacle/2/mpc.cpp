...
//    ocp.minimizeLagrangeTerm(coeffX*(sqrt(1+(x-xf)*(x-xf)) + sqrt(1+(y-yf)*(y-yf)) + sqrt(1+(z-zf)*(z-zf)) + r*r*50 + p*p*50 + q*q*50) + coeffU*( sqrt(1+(vu1)*(vu1))+sqrt(1+(vu2)*(vu2))+sqrt(1+(vu3)*(vu3))+sqrt(1+(vu4)*(vu4))));
...
//    h << exp((16 - (x*x+(z-5)*(z-5)))*0.01);
    ...
    Q(6,6) = coeffX*coeffX3;
    Q(7,7) = coeffX*coeffX3;
    Q(8,8) = coeffU;
    Q(9,9) = coeffU;
    Q(10,10) = coeffU;
    Q(11,11) = coeffU;
